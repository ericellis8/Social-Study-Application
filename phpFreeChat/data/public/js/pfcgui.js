...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    {
    //document.getElementById("save_group").style.display=xmlhttp.responseText;
    if(xmlhttp.responseText == "group"){
        document.getElementById("save_group").style.display = "";
        document.getElementById("save_class").style.display = "none";
    }else{
        document.getElementById("save_group").style.display = "none";
        document.getElementById("save_class").style.display = "";
    }
    }
  }
xmlhttp.open("GET","classOrGroup.php?group_name="+name,true);
xmlhttp.send();
},
  ...
  setTabById: function(tabid)
  {
    ...
    document.getElementById("pfc_title").innerHTML = this.current_tab;
    if(this.current_tab == "OSU Chat"){
        document.getElementById("save_group").style.display = "none";
  ...
  */
  ...
  createTab: function(name, tabid, type)
  {
   ...
   // $('pfc_channels_list').appendChild(br);
   // $('pfc_channels_list').appendChild(br2);
