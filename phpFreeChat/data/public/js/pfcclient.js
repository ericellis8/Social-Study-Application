...
xmlhttp.open("GET","addToBuddyList.php?name="+username,true);
xmlhttp.send();
},
...
*/
  ...
  updateNickWhoisBox: function(nickid)
  {
    ...
    if (pfc.getUserMeta(nickid,'nick') != this.nickname)
    {
      ...
      //add.setAttribute('href','addToBuddyList.php?name='+pfc.getUserMeta(nickid,'nick'));
      add.setAttribute('href', '#');
      add.setAttribute('onClick',"pfc.addBuddy('"+pfc.getUserMeta(nickid,'nick')+"')");
      //add.onclick = addBuddy(pfc.getUserMeta((nickid,'nick')));
      add.setAttribute('style', 'text-decoration:none');
      var img2 = document.createElement('img');
      img2.setAttribute('src', this.res.getFileUrl('images/pv-active.gif'));
      img2.alt = document.createTextNode('Add friend to Buddy List');
      ...
      p.appendChild(a);
      add.appendChild(document.createTextNode('Add user to buddy list'));
      addfriend.appendChild(add);
      div.appendChild(p);
  ...
  buildNickItem: function(nickid)
  {
    ...
    a.onclick = function(evt){
      ...
      d.style.left = (mousePosX(evt)-70)+'px';
